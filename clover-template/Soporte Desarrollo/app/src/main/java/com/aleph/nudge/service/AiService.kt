...
$modifierLines"""
    ...
    fun getSuggestion(currentItems: List<String>, callback: (Suggestion?) -> Unit) {
        ...
        Thread {
            try {
                ...
                val userPrompt = """Current order: $orderList
...
- Example reason: "Most people add fries with a burger""""
    ...
    private fun parseResponse(responseBody: String): Suggestion? {
        try {
            ...
            val suggestionJson = JsonParser().parse(jsonStr).asJsonObject
            ...
            if (!knownItems.containsKey(itemId)) {
                ...
                Log.w(TAG, "AiService: no match for '$itemName' either, returning best-effort suggestion")
            }
